...
## 2026-06-26 Batch 14b — C 端自助增量前端

### 「我的」hub 页 + Next <Link> 软导航避开硬导航水合竞态
blueprint §14.4 菜单 6 项，底部导航只放 4 项；我的权益/上课记录 作「我的」(profile)页的入口链接（Next `<Link>`，客户端软导航）→ 独立子页 /entitlements、/records。区别于底部导航的 `<a href>`（硬加载，有 14a 已知水合竞态 FR）——hub 用 `<Link>` 不受影响。新页够浅时优先「hub 入口」而非塞底部导航。

### 候补中 = 我的预约页内条件渲染另一实体
候补是 waitlist_entries 非 bookings；「我的预约」加 `候补中` filter（status==='waitlist'）时条件渲染 `useAppWaitlist()` 候补卡片而非 bookings。两 hook 都无条件调用（rules of hooks），按 filter 选渲染分支。候补卡片只有 session_starts_at(无 ends_at)→只格式化开始时间。useAppBookings 在候补模式传 '' 拉了一次没用的 bookings（可加 enabled 优化，FR）。

### 多状态 query 复用同一 hook
上课记录复用 `useAppBookings('attended,no_show')`——hook 把 status 原样进 query param，后端 split 逗号。无需新 hook/类型。失效键：我的权益用 `['app-entitlements']`（14a 预约/取消已预埋失效→下单后余额即时刷新）；加入/取消候补失效 `['app-waitlist']`+`['app-class-sessions']`。

## 2026-06-26 Batch 15 — 场次状态自动化（前端零改动）

### 提前埋好的状态渲染让后端自动化「白嫖」前端
Batch 15 后端给 `/schedule` 引入了真实的 `in_progress`/自动 `completed` 状态流，但**前端零改动**——`brand/app/(protected)/schedule/page.tsx` 早在 13e 就把 `in_progress:'进行中'`（绿徽标 `bg-green-100`）+ `completed:'已完成'` + 状态筛选项全部预置好了（当时没有任何场次会真的进入 in_progress，纯属前瞻）。教训：**状态机的 UI 标签/徽标/筛选一次性按完整枚举铺好，后续后端让该态真正产生时前端免改**。核对方式：批次起飞前 `grep "in_progress\|进行中"` 确认 UI 已覆盖全枚举，再决定是否需要前端 task。

## 2026-06-27 Batch 17（品牌运营看板 /reports）

### 看板页范式：useQuery 聚合端点 + 卡片/表，复用 Card/Table/Select 不引图表库
brand `/reports` 页：`useBrandReportOverview(params, enabled)`（params 含 range/from/to/location_id），标量指标用 Card 网格、热门课程/门店分布/教练场次用 Table。时间窗 Select（today/this_week/this_month/custom→2 个 date Input）+ 门店 Select。占座率 0..1 → `(r*100).toFixed(1)%`。§15 不做复杂图表，卡片数字 + 表足够。query 的 inline params 对象不会引发 refetch loop——react-query v5 默认 hashKey 按值（稳定 JSON 序列化）比较，非引用。

### enabled 门控数据请求 + 无权友好态（直链路由无 nav 保护）
nav 隐藏不挡直链路由：无 `report.view_basic` 用户直敲 /reports 仍渲染。页用 `!permsLoading && !canView` 守卫渲染"无权查看"卡片（permsLoading 时 canView fail-closed=false，但不闪拒绝卡而落到"加载中"）。**所有该页的数据 hook 都要 enabled=canView 门控**——code-review 抓到 `useBrandLocations` 无 enabled 会在拒绝卡渲染前多打一次 /locations（可能 403）；给共享 hook 加可选 `enabled` 形参（默认 true 向后兼容）即修。

### api 包孤立 typecheck 的 process 报错是 pre-existing（client.ts 缺 node types），以 brand build 为准
`pnpm --filter @mini-schedule/api typecheck` 报 `client.ts: Cannot find name 'process'`——api 包自身 tsconfig 不含 node types 的孤立现象，非本批回归。真正静态门是 `pnpm --filter @mini-schedule/brand build`（Next tsconfig 含 process/env 类型），它过即 OK（新页/hook/type 都在其中 typecheck）。
