...
## 2026-06-18 Batch 13b — code-review 修复 + 转 FR

### P1：产品 scope 编辑「停用门店/归档课程」chip 缺失（13a 同源，多选版）
EntitlementProductFormDialog 的 scope chip 只从 active 门店/published 课程拉；产品 scope 含已停用/归档项时 chip 缺失→看不到删不掉，且（后端原 active-only 校验下）保存撞 `ENTITLEMENT_SCOPE_INVALID`。修：locationOptions/courseOptions 并入 initial.scope_ids 里不在 active 列表的项（标「#id（已停用/已归档）」）+ 后端 scope 放宽存在性。

### P1：发放权益后产品页 issued_count 滞后
`useGrantEntitlement` 原只失效 `['learner-entitlements']`，未失效产品列表 → issued_count（反范式聚合）不刷新。修：grant onSuccess 额外失效 `['brand-entitlement-products']`。

### 转 FR
- 发放生效日 `new Date(\`${d}T00:00:00\`).toISOString()` 用本地时区→非 UTC 区差一天（与后端 per-brand TZ 一并做）。
- 产品次数 zod `min(0)`：填 0 只走顶层 apiError 无 inline 字段错（cosmetic）。

## 2026-06-22 Batch 13c — code-review 修复 + 转 FR

### P0（后端根因，前端侧现形）：domain struct 缺 json tag → GET 响应 PascalCase 与前端 snake_case 契约不符
`/booking-policy` 读取链路坏：后端 `domain/booking.Policy` 无 json tag → `GET /booking-policy` 返回 `{"BookAheadMinMinutes":...}`，前端 `BookingPolicy`/`toForm` 按 snake_case 读全 undefined → 输入框显示 "undefined"、复选框不勾、保存 NaN→null 污染。**注意单向**：PUT 方向好的（前端发 snake_case，后端 `policyBody` 有 json tag 能 bind），易被「PUT 能通」误判正常。**prod build 不查运行时 JSON 形状故没拦住**。修复点在后端补 json tag（前端类型是约定正确的一侧，勿改）。
**Pending exposure**：新端点若返回的是「无 json tag 的后端 domain struct」，前端按 snake_case 消费会静默拿 undefined。验收时对新 GET 端点 curl 核对响应 key 大小写，别只信 TS 类型（TS 不校验运行时形状）。

### 转 FR
- 代预约弹窗关闭态仍拉一次 `class-sessions`：`useBrandClassSessions(open?{...}:{})` 无 enabled 门（hook 本身不支持 enabled），对比同文件 `learnersQuery` 用了 `open` 作 enabled。仅多一次请求，非正确性问题。修需给 `useBrandClassSessions` 加 enabled 参数。
- `bookings.ts` 的 `useBrandBooking`/`getBooking`/`detail` key 本批无详情路由暂未用（映射真实 GET /bookings/:id 端点，非死码，留待详情页）。

## 2026-06-22 Batch 13d — 验收期修 2 项派生态 stale（UI 才暴露）

### drawer 头部冻结快照 + 行徽标 stale：单测/build 抓不到，只有交互 e2e 暴露
两个验收 finding 都是「派生态不刷新」：① WaitlistDrawer 头部容量用打开时快照，抽屉内转正后不更新（→useMemo 从 live items 派生）；② 场次行候补徽标不随 join/skip/cancel 刷新（→失效 brand-class-sessions）。**类型检查 + prod build 都过，纯交互态 stale 只有真实 e2e 点击流才现形**。规则：凡「模态/行展示的派生计数/容量」，验收必跑「操作后该数字是否即时刷新」这一步。

### 13c snake_case 契约 P0 经验生效：waitlist 类型一次对齐，无返工
本批 WaitlistEntry 前端类型与后端 domain Entry（带 json tag）一次对齐，code-review 零类型契约问题。延续 13c：新端点返回的 domain struct 后端建时即加 snake_case json tag，前端类型逐字段对（含 nullable）。

## 2026-06-25 Batch 14a — C 端 greenfield 既有阻断

### 登录 brand_id 发 string → 后端 int64 bind 失败「请求参数错误」（既有，已修）
AppLoginRequest.brand_id 是 string + 登录页 onSubmit 显式 String(data.brand_id) → 请求体 {brand_id:"21"} → 后端 WechatLoginRequest.BrandID int64 ShouldBindJSON 失败 → ErrInvalidRequest。zod 已 coerce.number，String() 多余且有害。修：AppLoginRequest.brand_id 改 number + 去 String()。C 端登录此前未端到端跑过故未暴露。
