import {
  ...
  type MessageCenterMetric,
  type MessageCenterReplyTemplate,
} from '@mini-schedule/admin-system/components/message-center'
import {
  getMessageCenterPreview,
  getMessageCenterSummary,
} from '@mini-schedule/admin-system/models/message-center'

export const adminMessageReplyTemplates: MessageCenterReplyTemplate[] = [
  {
    label: '确认处理中',
...
export const adminMessages: MessageCenterItem[] = [
  {
    id: 'msg-brand-audit',
    title: '新品牌入驻资料待复核',
    ...
    followUpAt: '今日 14:00',
    relatedRecords: [
      { label: '品牌资料', value: '青岚普拉提', href: '/brands', kind: '品牌' },
      { label: '平台管理员', value: '林晨', href: '/admins', kind: '员工' },
    ],
    slaLabel: '2 小时内',
    nextStep: '进入品牌详情页核对资料，确认无误后将品牌状态从 pending 调整为 active。',
    suggestedReply:
  ...
  },
  {
    ...
    followUpAt: '今日 17:00',
    relatedRecords: [
      { label: '品牌资料', value: '北岸健身', href: '/brands', kind: '品牌' },
      { label: '账号处理人', value: '周然', href: '/admins', kind: '员工' },
    ],
    slaLabel: '今日内',
    nextStep: '联系品牌联系人核验手机号，完成后在管理员管理中创建临时密码并通知对方更新。',
    suggestedReply:
  ...
  },
  {
    ...
    followUpAt: '',
    relatedRecords: [
      { label: '品牌资料', value: '云阶瑜伽', href: '/brands', kind: '品牌' },
      { label: '运营看板', value: '训练数据导出', href: '/dashboard', kind: '需求' },
    ],
    slaLabel: '2 个工作日',
    nextStep: '记录到后台报表需求池，回复当前版本暂不支持导出，并提供训练记录页筛选替代方案。',
    suggestedReply:
  ...
  },
  {
    ...
    followUpAt: '明日 10:00',
    relatedRecords: [
      { label: '品牌资料', value: 'Loop Studio', href: '/brands', kind: '品牌' },
      { label: '内容处理人', value: '唐雨', href: '/admins', kind: '员工' },
    ],
    slaLabel: '今日内',
    nextStep: '抽查课程详情页的 cover_url，若 URL 可访问则继续排查前端图片域名配置。',
    suggestedReply:
  ...
  },
  {
    ...
    followUpAt: '',
    relatedRecords: [
      { label: '平台概览', value: '品牌服务费发票', href: '/dashboard', kind: '系统' },
    ],
    slaLabel: '无需处理',
...
export const adminMessageSummary = getMessageCenterSummary(adminMessages)

export const adminMessageMetrics: MessageCenterMetric[] = [
  {
    label: '未读消息',
  ...
  {
    label: '处理中',
    value: String(adminMessageSummary.active),
    detail: '包含待跟进和已转交事项',
  },
  {
    label: '已解决',
    value: String(adminMessageSummary.resolved),
    detail: '当前示例队列中的闭环消息',
  },
]

export const adminMessagePreview = getMessageCenterPreview(adminMessages)
